...
export const examples = [
  {
    name: "FAB with Icon",
    Code: (
      function App() { const data = [ { uri: 'https://images.unsplash.com/photo-1599566150163-29194dcaad36?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774&q=80', name: 'Kevin James', msg: 'Hi Rachel, What\'s up', }, { uri: 'https://images.unsplash.com/photo-1527980965255-d3b416303d12?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1180&q=80', name: 'Jacob Jones', msg: 'Good Morning!', }, { uri: 'https://images.unsplash.com/photo-1633332755192-727a05c4013d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1180&q=80', name: 'Albert Flores', msg: 'Coffee?', }, ]; return (<Box className='border rounded-lg border-outline-200 p-6 bg-background-100 min-w-[240px] sm:min-w-[360px] md:min-w-[476px]' ><VStack space='md'>{data.map((chatData)=>{ return (<HStack space='sm' className='items-center'><Avatar size='sm'><AvatarImage source={{ uri: chatData.uri, }} /></Avatar><VStack><Heading size='xs'>{chatData.name}</Heading><Text size='xs'>{chatData.msg}</Text></VStack></HStack>); })}</VStack><Fab size='lg' className='bg-primary-600 hover:bg-primary-700 active:bg-primary-800' >{/* EditIcon is imported from 'lucide-react-native' */}<FabIcon as={EditIcon} color="white"/></Fab></Box>); }
    )
  },
  {
    name: "FAB with Icon and Text",
    Code: (
      <Box className='max-w-96 border rounded-lg border-outline-200 py-[56px] px-6 mx-5 bg-background-100' ><Fab className='top-4 h-9 bg-primary-600 hover:bg-primary-700 active:bg-primary-900'><FabIcon as={SearchIcon} /><FabLabel>Search</FabLabel></Fab><VStack space='lg'><Box><Text className='text-xs text-primary-600 font-bold'>HEALTH</Text><Heading size='sm'>Benefits of Oranges</Heading><Text size='xs' className='mt-1.5'>Oranges are a great source of vitamin C, which is essential for a healthy immune system.</Text><HStack space='sm' className='mt-3 h-5'><Text size='xs'>Wade Warrem</Text><Divider orientation='vertical' className='bg-background-300' /><Text size='xs'>6th Oct, 2019</Text><Divider orientation='vertical' className='bg-background-300' /><Text size='xs'>5 mins read</Text></HStack></Box><Divider /><Box><Text className='text-xs text-primary-600 font-bold'>TECHNOLOGY</Text><Heading size='sm' >How AI can benefit your business</Heading><Text size='xs' className='mt-1.5'>AI can automate tasks and processes, allowing for increasing efficiency and productivity.</Text><HStack space='sm' className='mt-3 h-5'><Text size='xs'>Wade Warrem</Text><Divider orientation='vertical' className='bg-background-300' /><Text size='xs'>6th Oct, 2019</Text><Divider orientation='vertical' className='bg-background-300' /><Text size='xs'>5 mins read</Text></HStack></Box></VStack></Box>
    )
  },
  {
    name: "Placement",
